...
#question{min-height:94px;font-size:1.02rem;line-height:1.4}.ask-button{font-size:1.02rem}.speak-button{margin-top:10px;background:#546e7a}.interpretation{font-size:.84rem;opacity:.72;margin:8px 0 0;line-height:1.4}
...
<form id="ask_form" class="card">
<p class="section-title">Demander à Élise</p>
<p class="section-sub">Écris ta question normalement. Pour un événement ancien, ajoute simplement une heure approximative.</p>
<label for="question">Ta question</label>
<textarea id="question" placeholder="Ex. Élise, pourquoi la lampe de la salle de bain vient de s'allumer ?"></textarea>
<button id="ask_btn" class="ask-button" type="submit">Demander à Élise</button>
</form>

<div id="result" class="card hidden"><div id="status" class="status"></div><p id="answer"></p><p id="interpretation" class="interpretation hidden"></p><div id="limits"></div><button id="speak_answer" type="button" class="speak-button hidden">🔊 Écouter la réponse</button><details><summary>Voir les preuves structurées</summary><pre id="json"></pre></details><div id="diag_actions" class="diag-actions hidden"><button id="copy_diag" type="button" class="diag-copy">Copier le diagnostic pour Élise</button><p class="diag-note">Pour analyse uniquement. Rien n'est envoyé automatiquement et les secrets connus sont retirés avant copie.</p></div></div>

<form id="form" class="card">
<p class="section-title">Mode précis</p>
<p class="section-sub">Pour cibler manuellement un objet, une heure, une valeur ou un attribut.</p>
...
<div class="card"><strong>Connexion IA</strong><p class="small">Une IA conversationnelle peut appeler <code>POST /api/v1/ask</code> avec une question naturelle, ou <code>POST /api/v1/investigate</code> avec des champs structurés. Le port réseau reste désactivé par défaut ; s'il est activé dans les réglages réseau de l'App, ce jeton protège l'API directe.</p><div id="token" class="token">Chargement…</div><p class="small">Schéma machine : <code>openapi.json</code></p></div>
</main>
<script>
const form=document.getElementById('form'), btn=document.getElementById('go');
const askForm=document.getElementById('ask_form'), askBtn=document.getElementById('ask_btn'), questionEl=document.getElementById('question');
const result=document.getElementById('result'), statusEl=document.getElementById('status'), answer=document.getElementById('answer'), jsonEl=document.getElementById('json'), limits=document.getElementById('limits'), interpretationEl=document.getElementById('interpretation'), speakAnswerBtn=document.getElementById('speak_answer');
...
function buildDiagnosticText(diagnostic){
 ...
 const version=response.meta?.version||'0.1.0-beta.11';
 return [
...
function resetResultExtras(){interpretationEl.textContent='';interpretationEl.classList.add('hidden');speakAnswerBtn.classList.add('hidden')}
function renderInvestigation(d,diagnostic,interpretationText=''){
 ...
 if(interpretationText){interpretationEl.textContent=interpretationText;interpretationEl.classList.remove('hidden')}else{interpretationEl.textContent='';interpretationEl.classList.add('hidden')}
 lastDiagnostic=diagnostic;if(d.status==='probable'||d.status==='indeterminate')diagActions.classList.remove('hidden');else diagActions.classList.add('hidden');
 if('speechSynthesis' in window&&answer.textContent)speakAnswerBtn.classList.remove('hidden');else speakAnswerBtn.classList.add('hidden');
 result.classList.remove('hidden');result.scrollIntoView({behavior:'smooth',block:'nearest'});
}
function renderError(message,candidates=[]){
 statusEl.textContent='À PRÉCISER';statusEl.className='status indeterminate';answer.textContent=message+(candidates.length?' Choix possibles : '+candidates.map(x=>x.name+' ('+x.entity_id+')').join(', ')+'.':'');jsonEl.textContent='';limits.innerHTML='';lastDiagnostic=null;diagActions.classList.add('hidden');resetResultExtras();result.classList.remove('hidden');
}
...
speakAnswerBtn.addEventListener('click',()=>{
 if(!('speechSynthesis' in window)||!answer.textContent)return;
 window.speechSynthesis.cancel();const utterance=new SpeechSynthesisUtterance(answer.textContent);utterance.lang='fr-FR';window.speechSynthesis.speak(utterance);
});
...
loadEntities();
askForm.addEventListener('submit',async e=>{e.preventDefault();const question=questionEl.value.trim();if(!question){renderError('Écris simplement ta question à Élise.');return}
 askBtn.disabled=true;askBtn.textContent='Élise enquête…';result.classList.add('hidden');diagActions.classList.add('hidden');resetResultExtras();lastDiagnostic=null;
 try{const r=await fetch(api('api/v1/ask'),{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({question})});const d=await r.json();if(!r.ok){renderError(d.error||'Je n’ai pas compris la question.',d.candidates||[]);return}
  const inv=d.investigation||{};const i=d.interpretation||{};let understood='Compris : '+(i.entity_name||i.entity_id||'objet');if(i.observed_value)understood+=' · valeur visée : '+i.observed_value;if(i.observed_time)understood+=' · heure interprétée : '+new Date(i.observed_time).toLocaleString('fr-FR');
  renderInvestigation(inv,{selection:{label:i.entity_name||i.entity_id,entity_id:i.entity_id},request:{question},interpretation:i,response:inv},understood);
 }catch(err){renderError(err.message||'Erreur de conversation')}finally{askBtn.disabled=false;askBtn.textContent='Demander à Élise'}
});
form.addEventListener('submit',async e=>{e.preventDefault();
 if(!entityEl.value){await loadEntities();const resolved=resolveTypedValue();if(resolved)setSelected(resolved)}
 if(!entityEl.value){renderError('Tape quelques lettres puis touche l’objet Home Assistant voulu dans la liste.');openPicker(findMatches(searchEl.value));return}
 btn.disabled=true;btn.textContent='Investigation…';result.classList.add('hidden');diagActions.classList.add('hidden');resetResultExtras();lastDiagnostic=null;
 ...
 renderInvestigation(d,{selection:{label:searchEl.value,entity_id:entityEl.value},request:body,response:d});
 }catch(err){renderError(err.message)}finally{btn.disabled=false;btn.textContent='Enquêter'}
});
</script></body></html>'''
