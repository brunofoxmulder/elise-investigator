...
from memory_worker_dev44 import TargetedConsciousMemoryWorker

VERSION = "0.2.0-dev.44"
CONSCIOUS_MEMORY_FILE = dev38.CONSCIOUS_MEMORY_FILE


def configure_dev44() -> None:
    """Keep dev.43 behavior and retry unresolved cover movement starts at terminal."""
    ...
    dev34._MEMORY_CARD = dev34._MEMORY_CARD.replace(
        "Mémoire consciente · dev.38", "Mémoire consciente · dev.44"
    )
...
async def create_app() -> web.Application:
    ...
    if isinstance(worker, (TargetedConsciousMemoryWorker, Dev43TargetedConsciousMemoryWorker)):
