...
class V2RC1PackagingTests(unittest.TestCase):
    def test_candidate_version_and_reader(self):
        self.assertEqual(main_v2_rc1.VERSION, "0.3.0-rc.1")
        self.assertIs(main_v2_rc1.ActivityTraceReaderV2, ActivityTraceReaderV2)

    def test_generic_launcher_points_only_to_v2_rc1(self):
        run = (ROOT / "elise_investigator" / "run.sh").read_text(encoding="utf-8")
        self.assertIn("exec python3 main_v2_rc1.py", run)
    ...
    def test_manifest_version_matches_candidate(self):
        ...
        self.assertIn('version: "0.3.0-rc.1"', config)

    def test_private_image_workflow_is_exactly_candidate_scoped(self):
        workflow = (ROOT / ".github" / "workflows" / "publish-v2-rc1-image.yml").read_text(encoding="utf-8")
        self.assertIn("candidate-v2-rc1", workflow)
        self.assertIn("elise-investigator-v2-rc1-private:0.3.0-rc.1", workflow)
